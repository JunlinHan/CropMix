...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_loader = torch.utils.data.DataLoader(
        datasets.CIFAR100('../data', train=True, download=True, transform= cropmix.CropMix(args.scale, args.mix_ratio, args.number, args.operation, args.inter_aug,  post_aug)),
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best, filename='checkpoint_{:s}.pth.tar'.format(args.arch))
            if epoch ==299:
                save_checkpoint({
                    'epoch': epoch + 1,
                    'arch': args.arch,
                    ...
                    'best_acc1': best_acc1,
                    'optimizer': optimizer.state_dict(),
                }, is_best,filename='checkpoint_299_{:s}.pth.tar'.format(args.arch))
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        args = parser.parse_args()
        shutil.copyfile(filename, 'model_best_{:s}.pth.tar'.format(args.arch))

...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="enable CropMix to generate new views of an image by mixing two "
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    augmentation_small = [
        transforms.RandomResizedCrop(224, scale=(0.2 ,0.6)),
        transforms.RandomApply(
            [transforms.ColorJitter(0.4, 0.4, 0.4, 0.1)],  # not strengthened
            p=0.8,
        ),
...
# --------------------------------------------------------------------------- #
    ...
    train_dataset = datasets.ImageFolder(
        ...
        moco.loader.CropsTransform(
            ...
            enable_cropmix=args.enable_cropmix,
            enable_multicrop=args.enable_multicrop,
            enable_mean_encoding=args.enable_mean_encoding,
            query_transform_small=transforms.Compose(augmentation_small),
            query_transform_large=transforms.Compose(augmentation_large),
        ),
    )
